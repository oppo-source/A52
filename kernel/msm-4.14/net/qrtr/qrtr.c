	...
	if (type == QRTR_TYPE_DATA) {
...
#else
        {
            QRTR_INFO(node->ilc,
				  ...
				  le32_to_cpu(pkt->server.node),
				  le32_to_cpu(pkt->server.port));
            oppo_match_qrtr_service_port(hdr->type, le32_to_cpu(pkt->server.service), le32_to_cpu(pkt->server.port));
        }
#endif /* VENDOR_EDIT */
...
static void qrtr_log_rx_msg(struct qrtr_node *node, struct sk_buff *skb)
{
	...
	if (cb->type == QRTR_TYPE_DATA) {
        ...
        oppo_match_qrtr_wakeup(cb->src_node, cb->src_port, cb->dst_port, (unsigned int)pl_buf, (unsigned int)(pl_buf >> 32));
#endif /* VENDOR_EDIT */
		QRTR_INFO(node->ilc,
...
#else
        {
            QRTR_INFO(node->ilc,
				  ...
				  le32_to_cpu(pkt->server.node),
				  le32_to_cpu(pkt->server.port));
            oppo_match_qrtr_service_port(cb->type, le32_to_cpu(pkt->server.service), le32_to_cpu(pkt->server.port));
        }
#endif /* VENDOR_EDIT */
