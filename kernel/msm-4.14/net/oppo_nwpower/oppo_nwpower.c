...
static void oppo_tcp_output_hook_work_callback(struct work_struct *work) {
	...
	if (oppo_tcp_output_hook.is_ipv6) {
		printk("[oppo_nwpower] IPAOutputWakeup: [%ld,%ld], %d, %d, %d",
			oppo_tcp_output_hook.ipv6_addr1, oppo_tcp_output_hook.ipv6_addr2,
			oppo_tcp_output_hook.pid, oppo_tcp_output_hook.uid, oppo_tcp_output_hook.set[3*i+2] & 0xFFFFFFFF);
	} else {
		printk("[oppo_nwpower] IPAOutputWakeup: %d, %d, %d, %d",
			oppo_tcp_output_hook.ipv4_addr, oppo_tcp_output_hook.pid, oppo_tcp_output_hook.uid,
...
static void oppo_tcp_input_hook_work_callback(struct work_struct *work) {
	...
	if (oppo_tcp_input_hook.is_ipv6) {
		printk("[oppo_nwpower] IPAInputWakeup: [%ld,%ld], %d, %d, %d",
			oppo_tcp_input_hook.ipv6_addr1, oppo_tcp_input_hook.ipv6_addr2,
			oppo_tcp_input_hook.pid, oppo_tcp_input_hook.uid, oppo_tcp_input_hook.set[3*i+2] & 0xFFFFFFFF);
	} else {
		printk("[oppo_nwpower] IPAInputWakeup: %d, %d, %d, %d",
			oppo_tcp_input_hook.ipv4_addr, oppo_tcp_input_hook.pid, oppo_tcp_input_hook.uid,
...
static void oppo_tcp_output_tcpsynretrans_hook_work_callback(struct work_struct *work) {
	...
	if (oppo_tcp_output_tcpsynretrans_hook.is_ipv6) {
		if (oppo_tcp_output_tcpsynretrans_hook.ipv6_addr1 == 0 && oppo_tcp_output_tcpsynretrans_hook.ipv6_addr2 == 0)
			return;
		printk("[oppo_nwpower] TCPOutputRetrans: [%ld,%ld], %d, %d, %d",
			oppo_tcp_output_tcpsynretrans_hook.ipv6_addr1, oppo_tcp_output_tcpsynretrans_hook.ipv6_addr2,
			oppo_tcp_output_tcpsynretrans_hook.pid, oppo_tcp_output_tcpsynretrans_hook.uid, oppo_tcp_output_tcpsynretrans_hook.set[3*i+2] & 0xFFFFFFFF);
	} else {
		if (oppo_tcp_output_tcpsynretrans_hook.ipv4_addr == 0)
			return;
		printk("[oppo_nwpower] TCPOutputRetrans: %d, %d, %d, %d",
			oppo_tcp_output_tcpsynretrans_hook.ipv4_addr, oppo_tcp_output_tcpsynretrans_hook.pid, oppo_tcp_output_tcpsynretrans_hook.uid,
...
static void oppo_tcp_input_tcpsynretrans_hook_work_callback(struct work_struct *work) {
	...
	if (oppo_tcp_input_tcpsynretrans_hook.is_ipv6) {
		if (oppo_tcp_input_tcpsynretrans_hook.ipv6_addr1 == 0 && oppo_tcp_input_tcpsynretrans_hook.ipv6_addr2 == 0)
			return;
		printk("[oppo_nwpower] TCPInputRetrans: [%ld,%ld], %d, %d, %d",
			oppo_tcp_input_tcpsynretrans_hook.ipv6_addr1, oppo_tcp_input_tcpsynretrans_hook.ipv6_addr2,
			oppo_tcp_input_tcpsynretrans_hook.pid, oppo_tcp_input_tcpsynretrans_hook.uid, oppo_tcp_input_tcpsynretrans_hook.set[3*i+2] & 0xFFFFFFFF);
	} else {
		if (oppo_tcp_input_tcpsynretrans_hook.ipv4_addr == 0)
			return;
		printk("[oppo_nwpower] TCPInputRetrans: %d, %d, %d, %d",
			oppo_tcp_input_tcpsynretrans_hook.ipv4_addr, oppo_tcp_input_tcpsynretrans_hook.pid, oppo_tcp_input_tcpsynretrans_hook.uid,
...
static void oppo_print_qrtr_wakeup(bool unsl) {
	...
	printk("[oppo_nwpower] AllWakeups: Modem: %d, Qrtr: %d, WiFi: %d, Adsp: %d, Cdsp: %d, Slpi: %d, Alarm: %d, IPCC: %d",
		nw_modem_wakeup_times, nw_qrtr_wakeup_times, nw_wifi_wakeup_times, nw_adsp_wakeup_times,
		nw_cdsp_wakeup_times, nw_slpi_wakeup_times, nw_alarm_wakeup_times, nw_ipcc_wakeup_times);
}
