...
extern struct work_struct oppo_tcp_input_tcpsynretrans_hook_work;
extern struct oppo_tcp_hook_struct oppo_tcp_input_tcpsynretrans_hook;
#endif /* VENDOR_EDIT */
...
queue_and_out:
	...
	if (!after(TCP_SKB_CB(skb)->end_seq, tp->rcv_nxt)) {
		...
		if (atomic_read(&tcpsynretrans_hook_boot) == 1) {
			...
			if (((now_ts.tv_sec * 1000 + now_ts.tv_nsec / 1000000) - sk->oppo_last_rcv_stamp[0]) > OPPO_TCP_RETRANSMISSION_INTERVAL) {
				if (sk->sk_v6_daddr.s6_addr32[0] == 0 && sk->sk_v6_daddr.s6_addr32[1] == 0) {
					oppo_tcp_input_tcpsynretrans_hook.ipv4_addr = sk->sk_daddr;
					oppo_tcp_input_tcpsynretrans_hook.is_ipv6 = false;
					schedule_work(&oppo_tcp_input_tcpsynretrans_hook_work);
				} else {
					oppo_tcp_input_tcpsynretrans_hook.ipv6_addr1 = (u64)ntohl(sk->sk_v6_daddr.s6_addr32[0]) << 32 | ntohl(sk->sk_v6_daddr.s6_addr32[1]);
					oppo_tcp_input_tcpsynretrans_hook.ipv6_addr2 = (u64)ntohl(sk->sk_v6_daddr.s6_addr32[2]) << 32 | ntohl(sk->sk_v6_daddr.s6_addr32[3]);
					oppo_tcp_input_tcpsynretrans_hook.is_ipv6 = true;
					schedule_work(&oppo_tcp_input_tcpsynretrans_hook_work);
				}
				oppo_tcp_input_tcpsynretrans_hook.uid = from_kuid_munged(&init_user_ns, sk->sk_uid);
				oppo_tcp_input_tcpsynretrans_hook.pid = sk->sk_oppo_pid;
			}
		}
#endif /* VENDOR_EDIT */
