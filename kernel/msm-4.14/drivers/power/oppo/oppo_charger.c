...
#define OPPO_CHGON_WAKEUP_DELAY_COUNT 4  //20S
...
batt_temp_zone_info_t batt_temp_zone_arr[BATT_STAT_INVALID];

void batt_temp_zone_arr_init(struct oppo_chg_chip *chip)
{
...
#ifdef VENDOR_EDIT
	/* daili@ODM.BSP.charge, 2020/1/2, Add for solve can't sleep problem start  */
	if (NULL == g_charger_chip)
		return -EINVAL;
	/* daili@ODM.BSP.charge, 2020/1/2, Add for solve can't sleep problem end*/	
#endif
	switch (psp) {
...
int oppo_chg_parse_charger_dt(struct oppo_chg_chip *chip)
{
...
#ifdef CONFIG_OPPO_SPECIAL__DEL
	...
	rc = of_property_read_u32(node, "qcom,qc_temp_normal_fastchg_ma_higt",
			&chip->limits.qc_temp_normal_fastchg_ma_higt);
	if (rc) {
		chip->limits.qc_temp_normal_fastchg_ma_higt = 2000;
	}
	...
	chg_err(
			...
			qc_temp_normal_fastchg_ma_higt = %d, \
			...
			if (chip->qc_9v2a_support) {
				...
				switch (chip->tbatt_status_9v2a) {
					case BATT_STAT_NORMAL_0:
						chip->limits.temp_normal_fastchg_current_ma = chip->limits.qc_temp_cool_fastchg_ma;
						break;
					case BATT_STAT_NORMAL:
						if(vbatt_higherthan_4180mv)
							chip->limits.temp_normal_fastchg_current_ma = chip->limits.qc_temp_normal_fastchg_ma_low;
						else
							chip->limits.temp_normal_fastchg_current_ma = chip->limits.qc_temp_normal_fastchg_ma_higt;
						break;
					case BATT_STAT_NORMAL_1:
						chip->limits.temp_normal_fastchg_current_ma = chip->limits.qc_temp_warm_fastchg_ma;
						break;
					case BATT_STAT_NORMAL_2:
						chip->limits.temp_normal_fastchg_current_ma = chip->limits.qc_temp_little_hot_fastchg_ma;
						break;
					case BATT_STAT_NORMAL_3:
						chip->limits.temp_normal_fastchg_current_ma = chip->limits.qc_temp_hot_fastchg_ma;
						break;
					default:
						break;
				}
...
static void oppo_chg_set_input_current_limit(struct oppo_chg_chip *chip)
{
	...
	if ((chip->led_on) && (chip->tbatt_status == BATTERY_STATUS__WARM_TEMP)) {
		vbus_limit = VBUS_LIMIT_5V;
	}
	charger_xlog_printk(CHG_LOG_CRTI,
		" led_on = %d, \
...
#ifdef VENDOR_EDIT
/* Liulitian@ODM.BSP.charge, 2020/3/31, for charging current incorrect when vbat is over 4180mV at temp 22~34. */
		charger_xlog_printk(CHG_LOG_CRTI, "++++++ 4180mv status changed ++++++\n");
		chip->vbat_4180mv_status_changed = true;
#endif
...
#ifdef VENDOR_EDIT
/* Liulitian@ODM.BSP.charge, 2020/3/30, 9V2A standard use 1C anti shake. */
...
#ifndef VENDOR_EDIT
/* Liulitian@ODM.BSP.charge, 2020/3/30, over temp recharge do not use anti shake. */
...
static OPPO_CHG_9V2A_TBATT_STATUS batt_temp_9v2a_stat_check(int temp, struct oppo_chg_chip *chip)
{
	OPPO_CHG_9V2A_TBATT_STATUS i;
	...
	for (i = BATT_STAT_COLD; i < BATT_STAT_WARM; ++i) {
		if (temp >= *(batt_temp_zone_arr[i].batt_temp_thres_addr) && temp < *(batt_temp_zone_arr[i + 1].batt_temp_thres_addr))
			return i;
	}
	return BATT_STAT_INVALID;
}

static void battery_temp_9v2a_anti_shake_handle(struct oppo_chg_chip *chip)
{
	int i;
	if (chip->tbatt_status_9v2a == BATT_STAT_NORMAL || chip->tbatt_status_9v2a == BATT_STAT_INVALID) {
		...
		for (i = BATT_STAT_COLD; i < BATT_STAT_INVALID; ++i) {
			if (i - 1 == chip->tbatt_status_9v2a)
				*(batt_temp_zone_arr[i].batt_temp_thres_addr) = batt_temp_zone_arr[i].default_value + HYSTERISIS_DECIDEGC;
			else
				*(batt_temp_zone_arr[i].batt_temp_thres_addr) = batt_temp_zone_arr[i].default_value;
		}
	}
...
static bool oppo_chg_check_tbatt_is_good(struct oppo_chg_chip *chip)
{
	...
	OPPO_CHG_9V2A_TBATT_STATUS tbatt_status_9v2a;
...
#ifdef VENDOR_EDIT
/* Liulitian@ODM.BSP.charge, 2020/3/31, for charging current incorrect when vbat is over 4180mV at temp 22~34. */
	chip->vbat_4180mv_status_changed = false;
#endif
	if (chip->qc_9v2a_support) {
...
static void oppo_chg_update_ui_soc(struct oppo_chg_chip *chip)
{
...
#ifdef VENDOR_EDIT
	/* Liulitian@ODM.BSP.charge, 2020/3/9, Add for charging status wrong in setting when plug out cable. START */
	int pre_prop_status = chip->prop_status;
	/* Liulitian@ODM.BSP.charge, 2020/3/9, Add for charging status wrong in setting when plug out cable. END */
#endif
	...
	if (chip->charger_exist && chip->batt_exist
		...
		if (chip->soc == chip->ui_soc) {
			...
			soc_up_count = 0;
#ifdef VENDOR_EDIT
/* Liulitian@ODM.BSP.charge, 2020/4/8, prevent ui_soc from decline when charging */
			if (chip->icharging >= 0)
#endif
				soc_down_count++;
...
static void battery_update(struct oppo_chg_chip *chip)
{
	...
	{
		iWakeUpDelay++;
		if(iWakeUpDelay >= OPPO_CHGON_WAKEUP_DELAY_COUNT)
		{
			iWakeUpDelay = 0;
		}
		else
...
#ifdef VENDOR_EDIT
/* Liulitian@ODM.BSP.charge, 2020/03/17, The status should be not charging when battery is over voltage */
...
#ifndef VENDOR_EDIT
/* wangdengwen@oppo.com, 2020/3/2, del for QC shake handle is active at oppo_chg_get_qc_tbatt_status() func */
static bool battery_qc_temp_anti_shake_handle(struct oppo_chg_chip *chip)
{
	int now_temp           = chip->temperature;
	static int pre_temp    = TBATT_PRE_SHAKE_INVALID;
    bool isNeedChangeStatus = false;
	
	if( likely( TBATT_PRE_SHAKE_INVALID != pre_temp) )
	{
		if (now_temp > pre_temp) 
		{ /*get warmer*/
			chg_debug("get warmer pre_temp=%d now_temp=%d\n",pre_temp, now_temp);
			isNeedChangeStatus = true;
		} 
		else if (now_temp < pre_temp - QC_TEMP_JEITA) 
		{ /*get cooler*/
			chg_debug("get cooler pre_temp=%d now_temp=%d\n",pre_temp, now_temp);
			isNeedChangeStatus = true;
		}
		else 
		{
...
void oppo_smart_charge_by_cool_down(struct oppo_chg_chip *chip, int val)
{
	...
	//changed by wangdengwen@oppo.com for QC3.0 smart charge
	if( !chip->chg_ctrl_by_vooc )
	{
		...
		switch ( val )
		{
			...
			case SMART_QC_CHARGER_5V1400MA:
				chip->limits.input_current_charger_ma    = OPPO_CHG_1400_CHARGING_CURRENT;
				chip->limits.pd_input_current_charger_ma = OPPO_CHG_1400_CHARGING_CURRENT;
				chip->limits.qc_input_current_charger_ma = OPPO_CHG_1400_CHARGING_CURRENT;
				chip->limits.qc_input_volt_charger_v     = VBUS_LIMIT_5V;
				break;
			...
			case SMART_QC_CHARGER_9V2000MA:
				...
				chip->limits.qc_input_volt_charger_v     = VBUS_LIMIT_9V;
				break;
			case SMART_QC_CHARGER_CELLMODE:
				chip->limits.input_current_charger_ma    = OPPO_CHG_500_CHARGING_CURRENT;
				chip->limits.pd_input_current_charger_ma = OPPO_CHG_500_CHARGING_CURRENT;
				chip->limits.qc_input_current_charger_ma = OPPO_CHG_500_CHARGING_CURRENT;
				chip->limits.qc_input_volt_charger_v     = VBUS_LIMIT_5V;
