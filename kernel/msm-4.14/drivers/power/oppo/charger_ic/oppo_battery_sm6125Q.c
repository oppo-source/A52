...
/* wangchao@ODM.BSP.charge, 2019/12/18, Add for QC charger factory test*/
	case POWER_SUPPLY_PROP_FAST_CHG_TYPE:
		val->intval = oppo_chg_get_charger_subtype();
		break;
#endif
	default:
...
static void oppochg_usbtemp_dischg(void)
{
	...
	//add by wangdengwen@oppo.com for bug(2661692) of 19375
	//-------------------------start-----------------------
	...
	while (!kthread_should_stop()) {
		if (oppochg_usbtemp_condition() == true) {
			...
			if(isQcTorchEn && !isVbusSet){
				...
				vBusSetEnd = false;
				oppochg_set_vbus(g_oppo_chip->limits.qc_input_volt_charger_v);
				isVbusSet = false;
				vBusSetEnd = true;
			}
#endif
...
static int oppochg_set_vbus(int vbus)
{
	...
	if (rc < 0) {
		...
		pr_debug("oppochg_set_vbus limit = %d",vbus);
	}
...
static int oppo_chg_set_pd_config(void)
{
	int ret = 0;
	struct oppo_chg_chip *chip = g_oppo_chip;
...
static bool oppo_sm7150_get_pd_type(void)
{
	struct smb_charger *chg = NULL;
...
struct oppo_chg_operations oppochg_smb5_ops = {
	...
	.oppo_chg_get_pd_type = oppo_sm7150_get_pd_type,
