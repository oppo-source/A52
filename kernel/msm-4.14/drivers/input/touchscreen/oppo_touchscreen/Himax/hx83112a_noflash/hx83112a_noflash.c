...
#define SKIP_NOTCH_CHECK_IN_DRIVER

/*******Part0:LOG TAG Declear********************/
#define TPD_DEVICE "himax,nf_hx83112a"
...
#ifdef VENDOR_EDIT
//Kui.Feng@BSP.TP.Function, 2020/01/08, add for black screen gesture--psensor related
...
static void himax_read_FW_ver(void)
{
    ...
    while(reload_status == 0) {
        ...
        if ((data[3] == 0xA9 && data[2] == 0x9A) || (data[3] == 0x3A && data[2] == 0xA3)) {
            TPD_INFO("reload OK! \n");
            reload_status = 1;
            break;
        } else if (retry == 0) {
    ...
    TPD_INFO("%s : 7f00,data[0] = 0x%2.2X, data[1] = 0x%2.2X, data[2] = 0x%2.2X, data[3] = 0x%2.2X\n", __func__, data[0], data[1], data[2], data[3]);
    TPD_INFO("%s : 72c0, data[0] = 0x%2.2X, data[1] = 0x%2.2X, data[2] = 0x%2.2X, data[3] = 0x%2.2X\n", __func__, data2[0], data2[1], data2[2], data2[3]);
...
static bool himax_reload_status_check(void)
{
    uint8_t cmd[4];
    uint8_t data[64];
    uint8_t data2[64];
    ...
    while(reload_status == 0)
    {
        ...
        cmd[0] = 0xc0;
        himax_register_read(cmd, 4, data2, false);
        TPD_INFO("%s now 7f00[0] = 0x%02X, [1] = 0x%02X,[2] = 0x%02X, [3] = 0x%02X; 72c0[0] = 0x%02X, [1] = 0x%02X\n"
		    ,__func__, data[0], data[1], data[2], data[3], data2[0], data2[1]);
        if ((data[3] == 0xA9 && data[2] == 0x9A) || (data[3] == 0x3A && data[2] == 0xA3)){
                reload_status = 1;//Reload OK
                break;
        } else if (retry == 0) {
                reload_status = 0;//Reload FAIL
                break;
        } else {
                retry--;
                msleep(10);
    ...
    TPD_INFO("%s, reload_status=%d, exit.\n",__func__, reload_status);
...
int himax_get_rawdata(struct chip_data_hx83112b *chip_info, int *RAW, uint32_t datalen)
{
    uint8_t tmp_addr[4];
    uint8_t tmp_data[4];
    uint8_t *tmp_rawdata;
...
{
#ifdef SKIP_NOTCH_CHECK_IN_DRIVER
	        return 0;
#else
        ...
        for (i = 0; i < (chip_info->hw_res->TX_NUM * chip_info->hw_res->RX_NUM); i++) {
            ...
            if(isRead_csv == false) {
                if ((int)RAW[i] > LPWUG_IDLE_NOISE_MAX || (int)RAW[i] < LPWUG_IDLE_NOISE_MIN) {
                    TPD_INFO("%s: HIMAX_INSPECTION_LPWUG_IDLE_NOISE,FAIL,inner,[%d]=%d, min=%d, max=%d\n",
                            __func__, i, RAW[i], LPWUG_IDLE_NOISE_MIN, LPWUG_IDLE_NOISE_MAX);
                    ret = THP_AFE_INSPECT_ENOISE;
                }
            } else {
                if ((int)RAW[i] > hx83112a_nf_inspection_criteria[IDX_LPWUG_IDLE_NOISE_MAX][i] ||
                    ...
                    TPD_INFO("%s: HIMAX_INSPECTION_LPWUG_IDLE_NOISE,FAIL,extand,[%d]=%d, min=%d, max=%d\n",
                            __func__, i, RAW[i], hx83112a_nf_inspection_criteria[IDX_LPWUG_IDLE_NOISE_MIN][i], hx83112a_nf_inspection_criteria[IDX_LPWUG_IDLE_NOISE_MAX][i]);
                    ret = THP_AFE_INSPECT_ENOISE;
                }
            }
        }
...
static void hx83112b_black_screen_test(void *chip_data, char *message)
{
    ...
    /*Init Log Data */
    hx83112b_enable_interrupt(chip_info, false);
...
void himax_get_DSRAM_data(uint8_t *info_data, uint8_t x_num, uint8_t y_num)
{
    ...
    for (i = 2; i < total_size; i = i + 2) { /* 2:PASSWORD NOT included */
        ...
        pr_cont("0x%2x:0x%4x ", temp_info_data[i], check_sum_cal);
        if (i % 32 == 0)
            pr_cont("\n");
    }
...
#ifndef VENDOR_EDIT
//Kui.Feng@BSP.kernel.Touch, 2019/12/20,Modify for himax diag node
/*
...
*/
#else
static size_t hx83112b_proc_diag_read(struct seq_file *s, void *chip_data)
{
    size_t ret = 0;
    uint16_t mutual_num;
    uint16_t self_num;
...
#else
    if (panel_data->manufacture_info.version)
	sprintf(&panel_data->manufacture_info.version[7], "%02X", panel_data->TP_FW);
...
static int hx83112b_tp_probe(struct spi_device *spi)
{
    ...
    chip_info->g_fw_buf = vmalloc(148 * 1024);
