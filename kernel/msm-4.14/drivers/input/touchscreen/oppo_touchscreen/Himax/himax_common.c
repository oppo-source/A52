...
#ifndef VENDOR_EDIT
//Kui.Feng@BSP.kernel.Touch, 2019/12/20,Modify for not creat no use tp testlimit file
    //fd = sys_open(data_buf, O_WRONLY | O_CREAT | O_TRUNC, 0);
#else
    fd = sys_open(data_buf, O_WRONLY | O_TRUNC, 0);
#endif
    if (fd < 0) {
...
static int himax_proc_diag_read(struct seq_file *s, void *v)
{
    ...
    ret = syna_ops->himax_proc_diag_read(s, ts->chip_data);
    return 0;
}

static int himax_diag_seq_open(struct inode *inode, struct file *file)
{
