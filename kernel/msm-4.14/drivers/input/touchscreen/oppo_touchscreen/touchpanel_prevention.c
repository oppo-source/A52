...
static int get_key_value(char *in, char *check)
{
    ...
    pos = strstr(in, check);
    if (!pos) {
        TPD_INFO("%s:can't find string:%s in %s.\n", __func__, check, in);
        return -1;
    }

    in_cnt = strlen(in);
    check_cnt = strlen(check);
    TPD_INFO("checklen:%d, inlen:%d.\n", check_cnt, in_cnt);
        ...
        if (OBJECT_PARAMETER == object) {
            if(OPERATE_MODIFY == cmd) {
                if ((value = get_key_value(split_str[i], "condition_frame_limit")) >= 0) {
                    ...
                    grip_info->grip_disable_level = value;
