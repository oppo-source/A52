...
static void tp_irq_thread_bannable(struct touchpanel_data *ts)
{
...
static void tp_irq_thread_unbannable(struct touchpanel_data *ts)
{
    if (ts->i2c_ready == false) {
        TPD_INFO("The device not resume 1000ms!");
        return;
...
static irqreturn_t tp_irq_thread_fn(int irq, void *dev_id)
{
    ...
    if (ts->int_mode == BANNABLE) {
        mutex_lock(&ts->mutex);
        tp_irq_thread_bannable(ts);
        mutex_unlock(&ts->mutex);
    } else {
        tp_irq_thread_unbannable(ts);
    }
...
OUT:
    ...
    ts->i2c_ready = true;
	mutex_unlock(&ts->mutex);
    if ((ts->black_gesture_support && (ts->gesture_enable & 0x01) == 1) ||
