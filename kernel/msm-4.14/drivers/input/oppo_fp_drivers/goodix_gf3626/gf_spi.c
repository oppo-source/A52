...
#define GF3626_SPIDEV_NAME     "goodix,goodix_fp_gf3626"
/*device name after register in charater*/
#define GF_DEV_NAME            "goodix_fp"
#define	GF_INPUT_NAME	    "qwerty"	/*"goodix_fp" */

#define	CHRD_DRIVER_NAME	"goodix_fp_spi"
#define	CLASS_NAME		    "goodix_gf3626"
...
#endif
{
	...
	if (of_find_property(pdev->dev.of_node, "fp_3v3-supply", NULL)) {
		printk("%s,find property fp_3v3-supply\n",__func__);
	}else{
		printk("%s,cannot find property fp_3v3-supply\n",__func__);
	}
	gf_regulator = devm_regulator_get(&gf_dev->spi->dev, "fp_3v3");
	if (IS_ERR(gf_regulator)) {
		printk("%s regulator_get failed\n",__func__);
	}else{
		printk("%s regulator_get succeed\n",__func__);
		regulator_enable(gf_regulator);
	}

	if (gf3626_parse_dts(gf_dev))
		goto error_hw;

	gf_hw_set_gpio_value(gf_dev->reset_gpio, 0);
