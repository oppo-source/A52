...
#ifdef VENDOR_EDIT
	/*Yongzhi.Zhang@PSW.MM.AudioDriver.CODEC, 2020/06/12, add for volume fadein*/
	bool fadein_enable;
...
static unsigned int dev_switch_us = 2*1000*1000; //2s
...
#ifdef VENDOR_EDIT
	/*Yongzhi.Zhang@PSW.MM.AudioDriver.CODEC, 2020/06/12, add for volume fadein*/
static void spk_aw_on(struct aw87339_t *aw87339)
{
	//Left spk on
	...
	if (do_fadein_flag) {
		...
		schedule_delayed_work(&aw87339->fadein_work, 0);
	}
	pre_fadein_flag = false;
}

static void rcv_aw_off(struct aw87339_t *aw87339)
{
	//RCV off, mark time
...
static void spk_aw_off(struct aw87339_t *aw87339)
{
	//Left spk off, cancel fadein
		pr_info("%s: cancel fadein_work\n", __func__);
		cancel_delayed_work_sync(&aw87339->fadein_work);
}

static void aw_fadein_work(struct work_struct *work)
{
	struct aw87339_t *aw87339 = container_of(work, struct aw87339_t, fadein_work.work);
	unsigned char vol_index = 0;
	unsigned char i = 0;
	unsigned int switch_interval = 0;

	pr_info("%s: do_fadein_flag %d\n", __func__, do_fadein_flag);
	...
	if ((spk_on_jiffies > rcv_off_jiffies)) {
		...
		if (do_fadein_flag && (switch_interval < spk_mute_us)) {
			pr_info("%s: need mute more %d ms temp not use\n", __func__, switch_interval/1000);
		}
	}

	pr_info("%s: go fadein, set gain 0\n", __func__);
	vol_index = aw87339_kspk_reg_val(aw87339_REG_GAIN,aw87339);
	i2c_write_reg(aw87339_REG_GAIN, 0, aw87339);
	for (i = 0; i < (vol_index + 1); i++) {
		if (!do_fadein_flag) {
			pr_info("%s: fadein stop vol_index=%d\n", __func__, vol_index);
			break;
		}
		//pr_info("%s: set vol=%d\n", __func__, i);
		i2c_write_reg(aw87339_REG_GAIN, i, aw87339);
		usleep_range(150*1000, 150*1000 + 50);
	}

	pr_info("%s: fadein done, recover gain\n", __func__);
	i2c_write_reg(aw87339_REG_GAIN, vol_index, aw87339);
...
#ifdef VENDOR_EDIT
	/*Yongzhi.Zhang@PSW.MM.AudioDriver.CODEC, 2020/06/12, add for volume fadein*/
	if ((aw87339 != aw87339_rec) || (!do_fadein_flag)) {
		i2c_write_reg(aw87339_REG_GAIN    , aw87339_kspk_reg_val(aw87339_REG_GAIN,aw87339    ), aw87339);
	} else {
		pr_info("%s: aw87339 setting gain to 0 when fadein\n", __func__);
		i2c_write_reg(aw87339_REG_GAIN, 0, aw87339);
	}
#else /* VENDOR_EDIT */
	i2c_write_reg(aw87339_REG_GAIN    , aw87339_kspk_reg_val(aw87339_REG_GAIN,aw87339    ), aw87339);
...
#ifdef VENDOR_EDIT
	/*Yongzhi.Zhang@PSW.MM.AudioDriver.CODEC, 2020/06/12, add for volume fadein*/
	if (aw87339_rec->fadein_enable) {
		spk_aw_on(aw87339_rec);
	}
#endif /* VENDOR_EDIT */
...
unsigned char aw87339_audio_spk_if_drcv(void)
{
    return aw87339_audio_drcv(aw87339_spk);
}
unsigned char aw87339_audio_abrcv(struct aw87339_t *aw87339)
{
    if(aw87339 == NULL) {
...
unsigned char aw87339_rcv_audio_drcv_off(void)
{
#ifdef VENDOR_EDIT
	/*Yongzhi.Zhang@PSW.MM.AudioDriver.CODEC, 2020/06/12, add for volume fadein*/
	if (aw87339_rec->fadein_enable) {
		rcv_aw_off(aw87339_rec);
	}
#endif /* VENDOR_EDIT */
...
static int aw87339_cfg_init(struct aw87339_t *aw87339)
{
	...
	/*Yongzhi.Zhang@PSW.MM.AudioDriver.CODEC, 2020/06/12, add for volume fadein*/
	if (aw87339 == aw87339_rec) {
		pr_info("%s: fadein enable for rec",
			__func__);
		aw87339->fadein_enable = true;
		INIT_DELAYED_WORK(&aw87339->fadein_work, aw_fadein_work);
	}
#endif /* VENDOR_EDIT */
    return ret;
}
...
EXPORT_SYMBOL(aw87339_audio_spk_if_drcv);
EXPORT_SYMBOL(aw87339_spk_audio_off);
#ifdef VENDOR_EDIT
/*Yongzhi.Zhang@PSW.MM.AudioDriver.CODEC, 2020/06/12, add for volume fadein*/
EXPORT_SYMBOL(aw87339_rcv_audio_drcv_off);
#endif /* VENDOR_EDIT */
