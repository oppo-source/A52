...
ssize_t sia81xx_tuning_if_dev_read(struct file *fp,
...
ssize_t sia81xx_tuning_if_dev_write(struct file *fp,
...
long sia81xx_tuning_if_dev_unlocked_ioctl(struct file *fp,
...
int sia81xx_tuning_if_dev_open(struct inode *inode, struct file *fp)
{
...
int sia81xx_tuning_if_dev_close(struct inode *inode, struct file *fp)
{
	pr_info("[ info][%s] %s: run !! \r\n", LOG_FLAG, __func__);
...
struct file_operations sia81xx_tuning_if_ops = {
	.owner = THIS_MODULE ,
...
struct miscdevice sia81xx_tuning_if_misc_dev = {
	.minor = MISC_DYNAMIC_MINOR ,
	.name = DEVICE_NAME ,
	.fops = &sia81xx_tuning_if_ops,
};

int open_tuning_if_dev(void)
{
	int ret_error ;
...
static int __init sia81xx_tuning_if_init(void)
{
	pr_info("[ info][%s] %s: run !! \r\n", LOG_FLAG, __func__);
...
static void __exit sia81xx_tuning_if_exit(void)
{
	pr_info("[ info][%s] %s: run !! \r\n", LOG_FLAG, __func__);
...
module_init(sia81xx_tuning_if_init);
module_exit(sia81xx_tuning_if_exit);

MODULE_AUTHOR("yun shi <yun.shi@si-in.com>");
MODULE_DESCRIPTION("SI-IN SIA81XX tuning interface");
MODULE_LICENSE("GPL");
