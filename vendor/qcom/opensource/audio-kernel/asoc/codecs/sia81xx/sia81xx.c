...
#define OWI_POLARITY				SIA81XX_DISABLE_LEVEL // 0 : pulse level == high,
														 // 1 : pulse level == low
...
//#define AFE_PORT_ID		AFE_PORT_ID_SLIMBUS_MULTI_CHAN_0_RX
	...
	struct sia81xx_dev_s *sia81xx)
{
	...
	//for dynamic auto set vdd to dsp
#endif
#if 1
	//mutex_lock(&sia81xx->mutex);

	atomic_set(&open_flag, 1);

	//if(1 == sia81xx->en_dyn_ud_vdd)
		sia81xx_enable_auto_set_vdd(sia81xx->dyn_ud_vdd_port);/* 0x400c SLIMBUS_6_RX */

	//mutex_unlock(&sia81xx->mutex);
#endif
	...
	struct sia81xx_dev_s *sia81xx)
{
	...
	//if(1 == sia81xx->en_dyn_ud_vdd)
		sia81xx_disable_auto_set_vdd(sia81xx->dyn_ud_vdd_port);/* 0x400c SLIMBUS_6_RX */
	...
	char* buf)
{
	...
	snprintf(tb, 1024, "sia81xx_cmd_show : owi pin : %d, chip id = 0x%02x \r\n"
		...
		"owi_write_data_cnt = %lu, open flag = %d, en vdd = %d \r\n",
		gpio_get_value(sia81xx->owi_pin), chip_id,
		...
		sia81xx->err_info.owi_write_data_err_cnt,
		sia81xx->err_info.owi_write_data_cnt,
		(int)atomic_read(&open_flag),
		(int)sia81xx->en_dyn_ud_vdd);
	strcpy(buf, tb);
	...
	size_t len)
{
	...
	switch(simple_strtoul(strsep(&cur, split_symb), &after, 10)) {
		...
		case SIA81XX_CMD_VDD_SET_OPEN :
		{
			...
			sia81xx_enable_auto_set_vdd(sia81xx->dyn_ud_vdd_port);
			//sia81xx->dyn_ud_vdd_port = 0x400c/* SLIMBUS_6_RX */;
			//sia81xx_open_set_vdd_server(0x400c/* SLIMBUS_6_RX */);
...
static int si_sia81xx_probe(struct platform_device *pdev)
{
	int ret = 0;
	sia81xx_dev_t *sia81xx = NULL;
	const char *chip_type_name = NULL;
...
static int si_sia81xx_remove(struct platform_device *pdev)
{
	int ret = 0;
	sia81xx_dev_t *sia81xx = NULL;
...
static struct platform_driver si_sia81xx_dev_driver = {
	.probe  = si_sia81xx_probe,
	.remove = si_sia81xx_remove,
	.driver = {
		.name = SIA81XX_NAME,
		.owner = THIS_MODULE,
