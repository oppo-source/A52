 ...
 * but must been modified at 4.16.0 + */
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4,14,0))
extern int q6audio_get_port_index(u16 port_id);
extern struct apr_svc *apr_register(char *dest, char *svc_name, apr_fn svc_fn,
                                   uint32_t src_port, void *priv);
extern int apr_send_pkt(void *handle, uint32_t *buf);
extern int msm_audio_ion_alloc(struct dma_buf **dma_buf, size_t bufsz,
                     dma_addr_t *paddr, size_t *plen, void **vaddr);
extern int msm_audio_ion_free(struct dma_buf *dma_buf);
extern int apr_deregister(void *handle);
#endif
...
static int afe_port_to_apr_port(
	uint16_t afe_port)
{
//	return q6audio_get_port_index(afe_port);
	...
	void *priv)
{
	...
	if(RESET_EVENTS == data->opcode) {
		...
		pr_debug("[debug][%s] %s: RESET_EVENTS, event : %d !! \r\n",
...
int __init sia81xx_afe_init(void) {
